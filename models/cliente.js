...
modelo.ofertasActivas=(idficha)=>{
	return new Promise(async (resolve,reject)=>{
		try {
			let comando="'S/. ' ||sum(numcuotasrestantes * cuotamensual) as suma, d.idsubasta from produccion.deuda d inner join produccion.subasta s on d.idsubasta = s.idsubasta where d.idsubasta is not null group by d.idsubasta";
			let resultado=await conexion.query(comando);
			resolve(resultado.rows);
		} catch (error) {
			reject(error);
